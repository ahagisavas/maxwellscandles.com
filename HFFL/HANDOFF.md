# Handoff Document — HFFL Fantasy Draft Assistant

**As of:** 2026-08-20, commit `c03f396` on `main`.
...
A live fantasy-football draft assistant, hardcoded to one specific league: the **Haraki Fantasy
Football League (HFFL)** — 12-team snake, you draft from **slot #11**. It ranks the full draftable
player pool in real time as picks come off the board, blending several ranking sources, and can
sync live picks automatically from a real Sleeper draft room.

**Live URL:** https://maxwellscandles.com/HFFL/
**Real draft date:** 2026-08-22.

## 2. Where everything lives

- **Repo:** https://github.com/ahagisavas/maxwellscandles.com (this HFFL tool is one folder in a
  larger personal-sites repo — Maxwell's Candles and other projects live in sibling folders, unrelated
  to this one).
...
### Module map (search for these section-divider comments in `index.html`)
| Section | Line (approx) | What it does |
|---|---|---|
| Hard-coded league config | 316 | `defaultSettings()` — team names/slots, roster, scoring rules. See §4. |
| Default player data | 368 | FantasyPros baseline projections, `DEFAULT_PLAYER_HEADERS`/`ROWS` |
| `CONSENSUS_DATA` | 1991 | FantasyPros ECR + Draft Sharks RK |
| `PROPS_DATA` | 2431 | BettingPros market prop lines — **see §5, this one has a parsing gotcha** |
| `FFBALLERS_DATA` | 2613 | The Fantasy Footballers' blended ranks |
| `DS_TIER_DATA` | 2931 | Draft Sharks tier/flag data |
| Small helpers | 975 | Misc utility functions |
| Scoring engine | 1020 | Turns raw stats into fantasy points per `settings.scoring` |
| Value engine (VBD) | 1108 | Replacement-baseline Value Based Drafting + tiering |
| Snake draft math | 1223 | Pick-number ↔ round/slot conversion for any `numTeams`/`userSlot` |
| Draft state | 1257 | Picks log is the single source of truth; everything else derives from it |
| Recommendation engine | 1547 | Staged, explainable pick suggestions |
| Importer | 1849 | Paste/CSV import with fuzzy column mapping (for one-off manual data adds) |
| Sleeper live draft sync | 4092 | Polls the real Sleeper API, auto-applies matched picks |
| Persistence | 4222 | localStorage autosave + JSON export/import |
| UI | 4269 | Rendering, event wiring |
| Main | 5294 | Bootstraps everything on load |

## 4. League configuration (all hardcoded in `defaultSettings()`, ~line 321)

- 12 teams, snake draft, **you are slot 11** ("Tasi's DOMATIA")
- Roster: 1 QB / 2 RB / 3 WR / 1 TE / 1 FLEX(RB/WR/TE) / 1 K / 1 DEF / 4 BN / 1 IR
- **Non-PPR** (`reception: 0` in scoring) — passing 20 yds/pt + 4/TD + -1/INT, rushing 10 yds/pt +
  6/TD, receiving 10 yds/pt + 6/TD, standard kicking/defense tiers.
- Real Sleeper draft_id confirmed live: `1387098256592367616`, 14 rounds (IR is a waiver-only slot,
  never drafted, so `totalRounds: 14` excludes it even though the roster has one).
- Team slot → manager name mapping is in the `teams:` array right below the roster/scoring config.

To reuse this tool for a different league/season: this whole block is meant to be edited directly
(there's no settings UI by design — see the comment right above it in the source).

## 5. Data pipeline — how the player pool gets built/refreshed

Four independent data sources get blended together; each has its own one-time or repeatable script,
all living next to `index.html`:
...
- **TEST vs. production, if this pattern ever gets reused elsewhere:** n/a to this specific tool, but
  see the general house rule in project memory — never touch a production data surface while
  diagnosing, only a test/diagnostic twin, unless told otherwise.
...
columns, badge system, favicon + web manifest (installable as a desktop/mobile app).
...
- **`HFFLDraft.url`** (Desktop) — shortcut to the live URL. Always current, needs internet.
- **`HFFLDraft (Offline Backup).html`** (Desktop, and a copy in `OneDrive\Documents\`) — a frozen,
  fully self-contained snapshot of `index.html` as of 2026-08-20. Works with zero internet (everything
  except live Sleeper sync). **This does not auto-update** — if `index.html` changes again after this
  date, this file needs to be manually re-copied to stay in sync. Check its top-of-file date/commit
  against this handoff doc's "As of" line if you're ever unsure how stale it is.
